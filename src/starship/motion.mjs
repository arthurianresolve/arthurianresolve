...
/** Bounds and rotation radius must describe the same currently visible effect. */
function envelopeAt(ship, time) {
  if (ship.shields?.some((shield) => time >= shield.hit && time <= shield.hit + SHIELD.duration)) return SHIELD;
  if (ship.burns?.some((burn) => time >= burn.start && time <= burn.end)) return DISCOVERY_EXHAUST;
  return { bounds: HULL_BOUNDS[ship.id], radius: HULL_RADII[ship.id] };
}
...
function hullsOverlap(a, poseA, marginA, b, poseB, marginB, time) {
  const corners = (ship, pose, margin) => {
    const [left, right, top, bottom] = envelopeAt(ship, time).bounds;
...
export function firstOverlapAt(a, b) {
  ...
  for (let i = 0; i < times.length; i++) {
    ...
    // Effect boundaries split the timeline, so each segment has one envelope.
    // A future shield must not enlarge rotation margins on an earlier clear pass.
    const radiusA = envelopeAt(a, (first + last) / 2).radius;
    const radiusB = envelopeAt(b, (first + last) / 2).radius;
